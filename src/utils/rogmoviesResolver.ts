...
export const getRogBaseDomain = (): string => getLiveDomain('rogmovies');
const BASE_DOMAIN = getLiveDomain('rogmovies');
const ROG_MIRRORS = [
  'https://rogmovies.rest',
  'https://rogmovies.im',
  'https://rogmovies.vip',
  'https://rogmovies.net',
];
...
/**
 * 2-Hop Gateway Resolver for RogMovies
 */
async function resolveActiveRogDomain(preferredDomain: string): Promise<string> {
  try {
    const res = await fetch(preferredDomain, { headers: { 'User-Agent': UA } });
    if (res.ok) {
      const html = await res.text();
      const liveEngineMatch = html.match(/href="([^"]*rogmovies\.[a-z0-9]+[^"]*)"/i) ||
  ...
  return ROG_MIRRORS[0];
}

async function fetchRogSearchHits(
  searchQuery: string,
  preferredDomain: string,
  signal?: AbortSignal
): Promise<{ hits: any[]; domain: string }> {
  const activeDomain = await resolveActiveRogDomain(preferredDomain);
  const domainsToTry = [
    activeDomain,
    ...ROG_MIRRORS.filter((d) => d !== activeDomain),
  ];
...
/**
 * Raw Search Cards Provider for RogMovies (Bollywood)
 */
export async function searchRogMoviesRawCards(
  queryTitle: string,
  baseDomain: string = BASE_DOMAIN,
  signal?: AbortSignal
): Promise<SearchArticleCard[]> {
  const cleanQ = sanitizeSearchQuery(queryTitle);
  const { hits, domain } = await fetchRogSearchHits(cleanQ, baseDomain, signal);
  ...
  return hits.map((hit: any, i: number) => {
    ...
    return {
      id: `rog-card-${i}-${Date.now()}`,
      title: postTitle,
      permalink,
      posterUrl: doc.post_thumbnail,
      siteKey: 'rogmovies',
      siteDisplayName: 'ROGMOVIES',
...
/**
 * 100% Robust RogMovies Article Parser with Strict Movie vs Series Segregation
 */
export function parseRogMoviesArticle(html: string, articleUrl: string = '', siteDisplayName: string = 'ROGMOVIES'): ScrapedQualityOption[] {
  ...
  sections.forEach((sec) => {
    ...
    links.forEach((l) => {
      ...
      options.push({
        id: `rog-${Math.random().toString(36).substr(2, 7)}`,
        siteKey: 'rogmovies',
        siteDisplayName,
...
/**
 * Multi-Page Search & Smart Verification Pipeline for RogMovies
 */
export async function getRogMoviesQualityOptions(
  ...
  const { hits, domain: activeDomain } = await fetchRogSearchHits(searchQuery, baseDomain, signal);
...
/**
 * 2-Tier Episode Discovery Engine for RogMovies (NexDrive Section Headers + V-Cloud #1 Priority)
 */
export async function fetchRogMoviesEpisodes(portalUrl: string): Promise<SeriesEpisodeItem[]> {
  try {
    ...
    sections.forEach((sec) => {
      ...
      for (const l of links) {
        ...
        if (href.startsWith('http') && !href.includes('imdb') && !href.includes('telegram')) {
          let priority = 99;
          if (/v-cloud|vcloud/i.test(btnText) || /vcloud|v-cloud/i.test(href)) priority = 1; // V-Cloud #1 Priority
  ...
  try {
    ...
    if (isTv) {
      ...
      const option480p = pool.find((o) => o.qualityLabel === '480p') ||
                         ...
                         pool[0];
      const resolved = await resolveRogMoviesLocker(option480p.targetUrl, option480p.qualityLabel || '480p');
